...
const AdminSetup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: fnError } = await supabase.functions.invoke("setup-first-admin", {
        body: { email, password, setup_key: setupKey },
      });
      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        ...
        >
          ...
          <p className="text-sm text-muted-foreground mb-6">
            Agora você pode fazer login com o e-mail e senha cadastrados.
          </p>
          <button
            onClick={() => navigate("/admin/login")}
            className="h-12 px-6 rounded-xl bg-gradient-gold-deep text-primary-foreground font-bold text-sm hover:opacity-90 transition-opacity"
          >
            Ir para o Login →
          </button>
        </motion.div>
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full h-12 rounded-xl bg-card border border-border px-4 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-gold/50 focus:shadow-gold transition-all"
        ...
        <button
          onClick={() => navigate("/admin/login")}
          className="mt-4 w-full text-center text-xs text-muted-foreground hover:text-foreground transition-colors flex items-center justify-center gap-1"
        >
          <ArrowLeft className="h-3 w-3" /> Já tenho uma conta
        </button>
      </motion.div>
    </div>
  );
