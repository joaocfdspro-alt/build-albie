...
interface DiagnosticChartProps {
  leads: { diagnostic_title: string }[];
}

const DIAGNOSTIC_COLORS: Record<string, { bar: string; text: string; bg: string }> = {
  "Voz Aprisionada": { bar: "bg-red-500", text: "text-red-400", bg: "bg-red-500/10" },
  "Voz Hesitante": { bar: "bg-amber-500", text: "text-amber-400", bg: "bg-amber-500/10" },
  "Voz em Despertar": { bar: "bg-blue-500", text: "text-blue-400", bg: "bg-blue-500/10" },
  "Voz Poderosa": { bar: "bg-emerald-500", text: "text-emerald-400", bg: "bg-emerald-500/10" },
};
...
export function DiagnosticChart({ leads }: DiagnosticChartProps) {
  const data = useMemo(() => {
    ...
    leads.forEach((l) => {
      counts[l.diagnostic_title] = (counts[l.diagnostic_title] || 0) + 1;
    });
    const total = leads.length || 1;
    const ordered = ["Voz Aprisionada", "Voz Hesitante", "Voz em Despertar", "Voz Poderosa"];
    return ordered
